...
bool RMDriver::SetCam()
{
    ...
    setAcquisitionFrameRate(cameraSptr, 200);
    //setExposureTime(cameraSptr,20000);
    //if(setResolution(cameraSptr,1280,720) != 0)
    //{
    //       return -1;
    //}
...
/**
 ...
 void RMDriver::imageConvert(Dahua::GenICam::CFrame &input, cv::Mat &originPut, cv::Mat &output) {
...
bool RMDriver::Grab(Mat &src)
{
    ...
    // }
    if (imageInit) {
            Mat cImageBGR(frame.getImageHeight(), frame.getImageWidth(), CV_8UC3);
            src = cImageBGR.clone();
            imageInit = false;
    }
    imageConvert(frame, originImage_, src);
    return true;
}
