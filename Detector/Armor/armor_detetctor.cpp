...
namespace rm{
    ...
    void ArmorDetector::MinLampDetect(Mat &roi) {
        ...
        for(auto &light : contoursLight) {
            ...
            if(avg_brightness[0] < 80) continue;
            ...
            circle(dst, pts_[0], 2, Scalar(0, 255, 0));
            putText(dst, to_string(num++), pts_[0],
                    FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255),
                    1);
        }
        /** match light **/
        MatchLamp(possible_lamps, dst);
        imshow("test",dst);
        waitKey();
    }
    /**
     * @brief reset min area rotate rect parameter
     * @details fit ellipse is much better than min area rect, this function is already abandoned
     * */
    void ArmorDetector::GetRealRect(RotatedRect &rect) {
     ...
     * */
     void ArmorDetector::MatchLamp(vector<Lamp> &possible_lamps, Mat &src) {
         vector<MatchLight> possible_match;
         for(int l1 = 0; l1 < possible_lamps.size(); l1++){
             ...
             if(pair.size()) {
                 ...
                 possible_match.emplace_back(ml);
                 if(l1) {
                     for(auto &i : possible_match) {
                         //cout << i.matchIndex2 << pair.back().matchIndex1 << pair.back().matchIndex1 << endl;
                         if(i.matchIndex2 == possible_match.back().matchIndex1 || i.matchIndex2 == possible_match.back().matchIndex2) {  // if twice judge share
                             cout << "score: " << possible_match.back().matchFactor << " " << i.matchFactor << endl;
                             if(possible_match.back().matchFactor < i.matchFactor) {
                                 possible_match.pop_back();
                                 break;
                             }
                             else if(possible_match.back().matchFactor > i.matchFactor){
                                 swap(i,possible_match.back());
                                 possible_match.pop_back();
                                 break;
                             }
                         }
                 ...
                 //cout << possible_lamps.size() << " " << possible_match.size() << endl;
             }
         }
         sort(possible_match.begin(), possible_match.end(), compMatchFactor);
         for(auto &match : possible_match) {
             Armor possible_armor(possible_lamps[match.matchIndex1], possible_lamps[match.matchIndex2]);
             for (int j = 0; j < 4; j++) {
                 line(src, possible_armor.pts[j], possible_armor.pts[(j + 1) % 4], Scalar(0, 255, 0), 1);
             }
             putText(src, to_string(match.matchFactor), possible_armor.pts[0],
                     FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255),
                     1);
         }
     }
