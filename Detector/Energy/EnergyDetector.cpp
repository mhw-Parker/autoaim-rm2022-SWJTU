    ...
    switch (mode) {
        case INIT:
            ...
            if(cnt_t++ > 10){
                ...
                energy_rotation_direction = cnt_i>5 ? 1 : -1;
                mode = STANDBY;
            }
            return false;
        case STANDBY :
...
void EnergyDetector::GetPredictPoint() {
    ...
    Eigen::MatrixXd rate = RMTools::LeastSquare(x_list,cut_filter_omega,1); //对上面数据用最小二乘
