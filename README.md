...
## 装甲版自瞄
- **说明：装甲板识别中有两套识的cpp分别是ArmorDetector.cpp和armor_detetctor.cpp，后者是后来重写的，两者公用一个头文件ArmorDetector.hpp**

- 特点：用传统算法实现了同一图像上的多目标同时识别及最优目标决策
...
```c++
	float maxArmorAngle = 45;//装甲板最大倾斜角度
        ...
        float minLightH2W = 0.5;//灯条最小允许长宽比
	float maxLightH2W = 40;//灯条最大允许长宽比
```
...
图像采集线程与检测线程并行可以提高整体帧率，但需要注意图像采集线程与陀螺仪数据接收线程的时间对齐问题，目前采用软同步方式，即接收数据后取最新的图像缓存。
