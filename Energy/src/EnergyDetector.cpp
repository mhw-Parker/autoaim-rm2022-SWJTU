...
/**
 * @brief
 * @remark 用图像坐标系直接计算yaw pitch
 */
void EnergyDetector::getImageDeltaXY(Point2f p){
	deltaX = (p.x - IMGWIDTH/2) * 0.2;
	deltaY = -(p.y - IMGHEIGHT/2) * 0.25;
}
void EnergyDetector::OutputAngle(Point2f p){
    deltaX = p.x - IMGWIDTH/2;
    deltaY = -(p.y - IMGHEIGHT/2);
	yaw = atan(deltaX / dpx) * 180 / PI - 4.4;
	pitch = atan(deltaY / dpy) * 180 / PI + 4.75;
}
